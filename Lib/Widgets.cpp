...
void Framer::Wheel(double spin, bool shift) {
	arcball.Wheel(spin, shift);
}
...
void Framer::Draw(mat4 fullview, bool showConstrainAxes) {
	if (arcball.m) {
		...
		Disk(MatrixOrigin(*arcball.m), 9, arcball.pink);
	}
}
	...
	if (mm) {
		...
		if (length(mouseDown-mouseMove) > 2)
			...
			for (int i = 0; i < 24; i++) {
				...
				if (i > 0) Line(s1, s2, lineWidth, mauve);
				s1 = s2;
			}
		...
		if (showConstrainAxes) {
			...
			if (use == Use::Camera) {
				if (!dragging || c.id == ConstraintIndex::YAxis)
					Line(vec2(center.x-radius, center.y), vec2(center.x+radius, center.y), lineWidth, c.id == ConstraintIndex::YAxis? yellow : mauve);
				if (!dragging || c.id == ConstraintIndex::XAxis)
					Line(vec2(center.x, center.y-radius), vec2(center.x, center.y+radius), lineWidth, c.id == ConstraintIndex::XAxis? yellow : mauve);
			}
			if (use == Use::Body) {
				for (int i = 0; i < 3; i++) {
...
void Button::Draw(GLuint textureName, GLuint textureUnit) {
	int y2 = y+h, x2 = x+w;
	vec3 p1((float)x,(float)y,0), p2((float)x,(float)y2,0), p3((float)x2,(float)y2,0), p4((float)x2,(float)y,0);
	Quad(p1, p2, p3, p4, textureName, textureUnit);
}
...
void Button::Draw(const char *nameOverride, float textSize, vec3 *colorOverride, vec3 textColor) {
	...
	size_t nchars = strlen(s), npixels = int(textSize*(float)nchars);
