...
GLuint GetShader() {
	if (!spriteShader) spriteShader = BuildSpriteShader();
	return spriteShader;
}

GLuint GetCollisionShader() {
	if (!spriteCollisionShader) spriteCollisionShader = BuildSpriteShader(true);
...
void Sprite::Wheel(double spin, bool shift) {
	scale += .1f*(float) spin;
