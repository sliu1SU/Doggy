...
vec2 ScreenPoint(vec3 p, mat4 m, int vp[4], float *zscreen) {
	vec4 xp = m*vec4(p, 1);
	if (zscreen)
		*zscreen = xp.z; // /xp.w;
...
vec2 ScreenPoint(vec3 p, mat4 m, float *zscreen) {
	int vp[4];
	glGetIntegerv(GL_VIEWPORT, vp);
	return ScreenPoint(p, m, vp, zscreen);
}
...
float ScreenDSq(double x, double y, vec3 p, mat4 m, float *zscreen) {
...
float ScreenDSq(int x, int y, vec3 p, mat4 m, float *zscreen) {
	return ScreenDSq((double) x, (double) y, p, m, zscreen);
}

float ScreenD(double x, double y, vec3 p, mat4 m, float *zscreen) {
	return sqrt(ScreenDSq(x, y, p, m, zscreen));
}

float ScreenD(int x, int y, vec3 p, mat4 m, float *zscreen) {
	return sqrt(ScreenDSq(x, y, p, m, zscreen));
}
...
void LineDash(vec3 p1, vec3 p2, float width, vec3 col1, vec3 col2, float opacity, float dashLen, float percentDash) {
	...
	float nDashes = totalLen/dashLen;
	vec3 seg = (p2-p1)/nDashes, dash = percentDash*seg;
	for (int i = 0; i < (int) nDashes; i++) {
