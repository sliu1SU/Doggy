...
vec3 RGBfromHSV(vec3 hsv) {
	// text expects h to be [0..360] but here use [0..1]
	hsv.x = fmod(360.f*hsv.x, 360.f);							// degrees cannot exceed 360
	if (abs(hsv.y) < FLT_MIN) return vec3(hsv.z, hsv.z, hsv.z); // color is on the black-and-white center line
	if (abs(hsv.x-360.f) < FLT_MIN) hsv.x = 0;					// change 360 to 0 degrees
	hsv.x /= 60.f;												// h now in [0,6]
	int i = (int) floor(hsv.x);									// floor returns the largest integer <= h
	float f = hsv.x-i, p = hsv.z*(1-hsv.y);
	float q = hsv.z*(1-(hsv.y*f)), t = hsv.z*(1-(hsv.y*(1-f)));
...
vec3 GetStockColor(int i) {
	int nColors = NStockColors();
	return staticNamedColors[i % nColors].color;
}
